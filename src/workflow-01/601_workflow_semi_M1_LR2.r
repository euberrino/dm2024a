...
TS_strategy_base9 <- function( pinputexps )
{
  ...
  param_local$train$final_train <- c(201901, 201902, 201903, 201904, 201905, 201906,
...
HT_tuning_base <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    extra_trees = FALSE,
    # Parte variable
    learning_rate = c( 0.9, 2 ),
    feature_fraction = c( 0.3, 0.8 ),
    num_leaves = c( 8L,  64L,  "integer" ),
    min_data_in_leaf = c( 500L, 10000L, "integer" )
  )
